...
def parse_bug_id(bug_id: str):
    ...
    if len(digits) == 2:
        # e.g. 'a01' → rep1, channel_id=None, csv_id=a01
...
def wdq_parser(bug_id: str, label_df: pd.DataFrame):
    ...
    try:
        ...
        if channel_id is None:
            ...
            if channel_id is None:
                if bug_id[1:] == "06": # hardcode for specific file w/ missing channel annotations
                    hardcoded_a06_map = {
                        "infec-grape": 2,
                        "clean-wild": 4,
                        "clean-grape": 6,
